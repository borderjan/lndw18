...
int write_data(int data){
    if(n_tx > 0){
        ...
        while(pmask < PIN_ALL){
            if(pmask & data_tx){
                gpio_write_pin(pin, (data & dmask)?HIGH:LOW);
                dmask << 1;
            }
            pmask << 1;
        }
...
int setup_robot(pinmask hs_rxpin, pinmask hs_txpin, pinmask data_rxpins, pinmask data_txpins){
    if(!issinglepin(hs_rxpin) || !issinglepin(hs_txpin)){
    ...
    gpio_set_pinmode(OUTPUT, hs_txpin | data_txpins);
    gpio_set_pinmode(INPUT, hs_rxpin | data_rxpins);
